...
-- PostgreSQL
-- Projeto de análise e qualidade de dados


-- Criação de uma única query para identificar o total de valores ausentes em todas as colunas.
...
-- Criação de uma query para identificar se em qualquer coluna há o caracter "?". 
...
-- Criação de uma query para identificar duplicatas (sem considerar a coluna id).
...
-- Criação de uma query para identificar duplicatas considerando as colunas (nome_campanha, data_inicio, publico_alvo, canais_divulgacao).
...
-- Criação de uma query que identifique outliers nas 3 colunas numéricas.    
-- (regras: media + 1.5 * desvio_padrão /// media - 1.5 * desvio_padrão)
